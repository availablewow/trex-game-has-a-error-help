...
function preload() {
  ...
  trex_jump = loadAnimation("trex4.png");
...
function setup() {
  ...
  ground = createSprite(width / 2, height - 260, width, 20);
...
function draw() {
  ...
  fill("black")
  textFont("papyrus");
  text("HIGHSCORE : " + localStorage["HighScore"], width - 450, 50);
  ...
  if (gameState === PLAY) {
    ...
    if (trex.isTouching(cactiGroup) || trex.isTouching(pteradactylGroup)) {
      gameState = END;
      gameoversound.play();
    ...
    if (mousePressedOver(restart) || touches.length > 0||keyWentUp("space")) {
      reset();
      touches = [];
    }
  }
...
function spawnObstacle2() {
  if (frameCount % 110 === 0) {
    cactus5 = createSprite(width + 400, Math.round(random(height - 350, height - 500)), 40, 10);
